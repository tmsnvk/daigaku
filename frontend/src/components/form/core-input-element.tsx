...
const coreInputElementVariants = cva(
  joinTw(
    'w-[95%] sm:w-[65%] h-[5rem] px-4 text-xl border-2 border-solid border-secondary rounded-xl',
 ...
 */
interface CoreInputElementProps<T extends FieldValues> extends VariantProps<typeof coreInputElementVariants> {
  /**
...
export const CoreInputElement = <T extends FieldValues>({
  ...
  return (
    <input
      ...
      disabled={isDisabled}
      autoComplete={'off'}
      className={joinTw(coreInputElementVariants({ intent, className }), isError && 'border-destructive focus:outline-destructive')}
    />
  );
};
