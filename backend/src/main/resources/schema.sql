...
DROP TABLE IF EXISTS institutions_admins CASCADE;
...
CREATE TABLE institutions_admins(
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  account_id BIGINT REFERENCES accounts(id)
);
...
CREATE TABLE accounts_institutions_admins_junction(
  account_id BIGINT REFERENCES accounts(id) PRIMARY KEY,
  institution_admin_id BIGINT UNIQUE REFERENCES system_admins(id) NOT NULL
);
